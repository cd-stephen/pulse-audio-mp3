...
int main(int argc, char *argv[])
{
  ...
  while (mpg123_read(mh, buffer, buffer_size, &done) == MPG123_OK)
  {
    ...
    for (size_t i = 0; i < done && i < 1024; ++i)
    { 
      printf("%02x ", buffer[i]);
    }
    printf("\n");
    buffer_count++;
    printf("##### buffer Done ###### KBytes Processed: %d  MBytes Processed: %d", buffer_count, (buffer_count / 1024));
    printf("\n");
...
// gcc mp3player-streamdata.c -o mp3player-stream -lmpg123 -lpulse-simple -lpulse
